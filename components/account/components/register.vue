<template>
  ...
  <section class="register-box">
    <div class="hd">
      ...
      <p class="sub-title">
        已有账号
        <a class="link" @click="$emit('switch', 'login')">去登录</a>
      </p>
    </div>
    <div class="bd">
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <template v-if="step === 0">
          ...
          <el-form-item label="请输入短信验证码" prop="captcha">
            ...
            <button
              ...
              @click.stop.prevent="sendCaptcha(1)"
            >
        ...
        <!-- 注册阶段 -->
        <template v-else>
          <el-form-item label="请设置登录密码" prop="pwd">
            <el-input
              v-model="form.pwd"
              type="password"
          ...
          <el-form-item label="请再次确登录密码" prop="rePwd">
            <el-input
              v-model="form.rePwd"
              type="password"
        ...
        <!-- 协议 -->
        <el-form-item>
          <el-checkbox v-model="form.agree" class="note">
...
export default {
  ...
  computed: {
    disabledCaptcha() {
      return !PhoneRegex.test(this.form.phone) || this.countdown > 0
    },
    disabledVerify() {
      return !PhoneRegex.test(this.form.phone) || !this.form.captcha
    },
    disabledRegister() {
  ...
  methods: {
    ...
    async verifyCaptcha() {
      ...
      // 校验验证码
      // 校验成功，保存签名
      if (!this.form.agree) return // TODO 提示同意相关协议
    ...
    async register() {
      ...
      if (this.form.pwd !== this.form.rePwd) return // TODO 提示密码不一致
      ...
      const { code, data, message } = await this.$store.dispatch(
        ...
        {
          phone: this.form.phone,
          pwd: SparkMD5.hash(this.form.pwd),
